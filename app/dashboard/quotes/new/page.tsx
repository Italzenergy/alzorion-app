...
import { Trash2, Save, X, Search, UserCheck, CheckCircle, AlertCircle, Info } from 'lucide-react';
...
export default function NewQuotePage() {
  ...
  const [activeTaxRow, setActiveTaxRow] = useState<number | null>(null);
  ...
  //  1. NUEVO ESTADO PARA LOS COMENTARIOS 
  const [notes, setNotes] = useState('Condiciones de pago: 50% anticipo, 50% contraentrega.');

  const [highlightIndex, setHighlightIndex] = useState<number | null>(null);
  const [items, setItems] = useState<any[]>([
    { product_id: '', product_name: '', internal_code: '', accounting_ref: '', search_query: '', quantity: 1, unit_price: 0, tax_rate: 0.19, total_price: 0 }
  ]);

  // Estados para modales de feedback y confirmación
  ...
  const templates = [
    {
      name: "Kit Estructura Coplanar",
      items: ["END CLAMP", "MID CLAMP", "GROUNDING LUG", "RIELES A2", "RAIL SPLICE", "L FEET"] 
    },
    {
      name: "Kit Básico Inversor",
      items: ["INVERSOR 5KW", "CABLE SOLAR 4MM", "CONECTOR MC4"]
    }
  ];
  ...
  const loadTemplate = (templateName: string) => {
    ...
    setConfirmDialog({
      ...
      message: `¿Deseas cargar la plantilla "${templateName}"? Esto agregará los productos correspondientes a la cotización.`,
      type: 'info',
      onConfirm: () => {
        ...
        if (newItemsToLoad.length === 0) {
          setModalFeedback({
            isOpen: true,
            type: 'error',
            message: `No se encontraron productos en el catálogo para la plantilla "${templateName}".`
          });
          return;
        }
        ...
        setModalFeedback({
          isOpen: true,
          type: 'success',
          message: `Plantilla "${templateName}" cargada correctamente.`
        });
      }
    });
  };
  ...
  const handleSave = async () => {
    if (!client.name || !client.nid) return alert("Falta el Nombre o NIT del cliente.");
    if (items.some(i => !i.product_id || i.unit_price <= 0)) return alert("Revisa los productos: Faltan datos o precios.");
    ...
    try {
      const payload = { 
        ...
        //  2. PASAMOS EL ESTADO DE LOS COMENTARIOS AL BACKEND 
        notes: notes 
      };
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}><h1 className={styles.title}>Nueva Cotización</h1></div>
      <div className={styles.actionBar}>
        <button className={styles.btnPrimary} onClick={handleSave} disabled={loading}>
          <Save size={18} style={{ display: 'inline', marginRight: '5px', verticalAlign: 'middle' }}/> {loading ? 'Guardando...' : 'Confirmar y Guardar'}
        </button>
        <button className={styles.btnSecondary} onClick={() => router.push('/dashboard/quotes')}>Descartar</button>
      </div>
      ...
      <div className={styles.card}>
        <div className={styles.grid2}>
          <div>
            ...
            <div className={styles.formGroup}><label>Dirección (Opcional)</label><input type="text" className={styles.input} value={client.address} onChange={e => setClient({...client, address: e.target.value})} /></div>
          </div>
          <div>
        ...
        {/*  3. CUADRO DE TEXTO PARA LOS COMENTARIOS EN PANTALLA */}
        ...
        <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
          {templates.map(template => (
            <button 
              ...
              style={{
                backgroundColor: '#2a2a2a', color: '#f4f4f4', border: '1px solid #444', 
                padding: '8px 15px', borderRadius: '6px', cursor: 'pointer', fontSize: '14px'
              }}
            >
      ...
      {/* MODALES DE FEEDBACK Y CONFIRMACIÓN */}
